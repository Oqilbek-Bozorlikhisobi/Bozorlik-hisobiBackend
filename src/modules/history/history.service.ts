...
} from './exeptions/history.exeption';
...
export class HistoryService implements IHistoryService {
  ...
  async create(dto: CreateHistoryDto): Promise<ResData<History>> {
    ...
    newHistory.marketType = market.marketType;
  ...
  > {
    const result = await this.historyRepository.findAll(userId, query);
    return new ResData('ok', 200, result);
  }
